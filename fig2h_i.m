...
%% Let us compare the average decoding error using the same method for 30 trials
display('Computing decoding error using uniform prior')
for trial_i = 1:30
    ...
    decoding_agreement_uniform(trial_i) = sum(decoding_agreement_vector)./length(decoding_agreement_vector);
    decoding_error = actual_position - decoded_position;
    mean_decoding_error_uniform(trial_i) = mean(abs(decoding_error), 'omitnan');
end

%% Now let us do the same using an observed prior when computing bayesian probabilities
display('Computing decoding error using observed prior')
for trial_i = 1:30
    disp(['Trial ' num2str(trial_i) ' out of 30'])
    training_ts = create_training_set(ca_time, training_set_creation_method, training_set_portion);
    training_ts(running_ts == 0) = 0; 
    ...
    decoding_agreement_vector(isnan(actual_bin)) = nan;
    decoding_agreement_vector(isnan(decoding_agreement_vector)) = [];
    decoding_agreement_observed(trial_i) = sum(decoding_agreement_vector)./length(decoding_agreement_vector);
    decoding_error = actual_position - decoded_position;
    mean_decoding_error_observed(trial_i) = mean(abs(decoding_error), 'omitnan');
end

%% Same using unbiased bayesian probabilities
display('Computing decoding error using unbiased bayesian probabilities')
for trial_i = 1:30
    disp(['Trial ' num2str(trial_i) ' out of 30'])
    training_ts = create_training_set(ca_time, training_set_creation_method, training_set_portion);
    training_ts(running_ts == 0) = 0; 
    ...
    prob_being_active = prob_being_active./prob_being_active; % This will remove the activity bias
    decoding_ts = ~training_ts;
    decoding_ts(running_ts == 0) = 0;
    cell_used = logical(ones(size(ca_data,2),1)); 
    [decoded_probabilities] = bayesian_decode1D(binarized_data, occupancy_vector, prob_being_active, tuning_curve_data, cell_used);
    ...
    decoding_agreement_vector(isnan(actual_bin)) = nan;
    decoding_agreement_vector(isnan(decoding_agreement_vector)) = [];
    decoding_agreement_unbiased(trial_i) = sum(decoding_agreement_vector)./length(decoding_agreement_vector);
    decoding_error = actual_position - decoded_position;
    mean_decoding_error_unbiased(trial_i) = mean(abs(decoding_error), 'omitnan');
end