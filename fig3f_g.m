...
%% Let us compare the average decoding error using different training portions
training_port_vec = 0.1:0.1:0.9;
for training_port_i = 1:length(training_port_vec)
    display(['Computing decoding error using a ' num2str(training_port_vec(training_port_i)) ' portion of the data to train the decoder'])
    for trial_i = 1:30
        disp(['Trial ' num2str(trial_i) ' out of 30'])
        training_ts = create_training_set(ca_time, training_set_creation_method, training_port_vec(training_port_i));
        ...
        decoding_agreement_training_portion{training_port_i}(trial_i) = sum(decoding_agreement_vector)./length(decoding_agreement_vector);
        decoding_error = actual_position - decoded_position;
        mean_decoding_error_training_portion{training_port_i}(trial_i) = mean(abs(decoding_error), 'omitnan');
    end
end
