function [MI, posterior, prior, marginal_likelihood, likelihood ] = extract_1D_information(binarized_trace, interp_behav_vec, bin_vector, inclusion_vector)
...
%% Create bin vectors
marginal_likelihood = sum(binarized_trace)./length(binarized_trace); % Expressed in probability of firing (<1)
...
for i = 1:length(bin_vector)-1
    ...
    if ~isempty(position_idx)
        ...
        prior(i) = length(position_idx)/length(binarized_trace);
        ...
        if joint_prob_active ~= 0
            MI = MI + joint_prob_active*log2(joint_prob_active./(prob_in_bin*marginal_likelihood));
        end
        if joint_prob_inactive ~= 0
            MI = MI + joint_prob_inactive*log2(joint_prob_inactive./(prob_in_bin*(1-marginal_likelihood)));
        end
    end
end
