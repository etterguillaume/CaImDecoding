...
training_port_vec = 0.5;
smoothing_vec = 0:0.1:1;

%% Let us compare the average decoding error using different number of cells
for smooth_i = 1:length(smoothing_vec)
    display(['Computing decoding error with gaussian smoothed tuning curves using sigma = ' num2str(smoothing_vec(smooth_i))])
    for trial_i = 1:30
        ...
        for cell_i = 1:size(binarized_data,2)
            ...
            tuning_curve_data(:,cell_i) = filter(gausswin(smoothing_vec(smooth_i)),1,tuning_curve_data(:,cell_i));
        end
        ...
        decoding_agreement_gauss_smooth{smooth_i}(trial_i) = sum(decoding_agreement_vector)./length(decoding_agreement_vector);
        decoding_error = actual_position - decoded_position;
        mean_decoding_error_gauss_smooth{smooth_i}(trial_i) = mean(abs(decoding_error), 'omitnan');
    end
end