function [MI, posterior, prior, marginal_likelihood, likelihood ] = extract_2D_information(binarized_trace, interp_behav_vec, X_bin_vector, Y_bin_vector, inclusion_vector)
...
%% Create bin vectors
marginal_likelihood = sum(binarized_trace)./length(binarized_trace);
...
for y = 1:length(Y_bin_vector)-1
    for x = 1:length(X_bin_vector)-1
        ...
        if ~isempty(position_idx)
            ...
            prior(y,x) = length(position_idx)/length(binarized_trace);
